...
bool creaProjectil(ArrayProjectils &llistaProjectils,float x, float y, float angle)
{
	...
	if (llistaProjectils.numProjectils<MAX_PROJECTILS) {
		llistaProjectils.DadesProjectil[llistaProjectils.numProjectils].posicioX = x - 10*(float)cos((float)angle * PI / 180.0) * 0.01 * 3;
		llistaProjectils.DadesProjectil[llistaProjectils.numProjectils].posicioY = y - 10*(float)sin((float)angle * PI / 180.0) * 0.01 * 3;
